...
class CustomListTileWithDate extends StatefulWidget {
  const CustomListTileWithDate({
    ...
    required this.title,
    this.forEdit = true,
    this.initialDateTime, // String parameter for initial date
  });

  final String title;
  final bool forEdit;
  final String? initialDateTime; // Nullable String for initial date
...
class _CustomListTileWithDateState extends State<CustomListTileWithDate> {
  ...
  void initState() {
    super.initState();
    // Try to parse initialDateTime if provided; fallback to DateTime.now()
    selectedDateTime = widget.initialDateTime != null
        ? DateTime.parse(widget.initialDateTime!) // Parse ISO 8601 date string
        : DateTime.now();
  }
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  widget.forEdit
                      ...
                      : const SizedBox(), // Empty widget when forEdit is false
                ],
              ),
            ),
          ),
